...
def extract_and_transform():
    ...
    for current_csf_name, site_code in SITES_CONFIG.items():
        ...
        for bureau_prefix, service_label in BUREAUX_CONFIG.items():
            ...
            try:
                ...
                # v3.1 DEBUG — traçage output_id à ce stade précis
                if 'output_id' in df_service.columns and 'vrai_type' in df_service.columns:
                    n_liees = df_service[
                        (df_service['vrai_type'] == 'Entrée') & (df_service['output_id'].notna())
                    ].shape[0]
                    print(f"    🔎 DEBUG [{db_name}] Entrées liées à une sortie (output_id) après dedup : {n_liees}")
    ...
    # v3.1 DEBUG
    if 'output_id' in df_total.columns and 'vrai_type' in df_total.columns:
        n = df_total[(df_total['vrai_type'] == 'Entrée') & (df_total['output_id'].notna())].shape[0]
        print(f"🔎 DEBUG df_total juste après concat : {n} Entrées avec output_id")
    ...
    # v3.1 DEBUG
    if 'sortie_record_date' in df_total.columns:
        n_sd = df_total['sortie_record_date'].notna().sum()
        n_rd = df_total['registration_date'].notna().sum()
        print(f"🔎 DEBUG après to_datetime : sortie_record_date non-null={n_sd}, registration_date non-null={n_rd}")
    ...
    #     return delta_h if delta_h >= 0 else None  # négatif = anomalie, déjà tracée à part
        ...
        # =================================================================
    # v3.2 — DURÉE DE NUMÉRISATION (Sortie is_for_scan, retour - indexation)
    # =================================================================
    def calcul_duree_numerisation(row):
        """
        Durée de scan : date_retour - date_indexation
        Uniquement pour les SORTIES avec is_for_scan = True.
        """
        if str(row.get('vrai_type', '')).strip() != 'Sortie':
            return None
        if row.get('is_for_scan') != True:
            return None
        
        date_retour = row.get('date_retour')
        date_indexation = row.get('date_indexation')
        
        if pd.isna(date_retour) or pd.isna(date_indexation):
            return None
        
        delta_h = (date_retour - date_indexation).total_seconds() / 3600
        return delta_h if delta_h >= 0 else None  # négatif = anomalie

    df_total['duree_numerisation_h'] = df_total.apply(calcul_duree_numerisation, axis=1)
    # Force le type numérique pour DuckDB
    df_total['duree_numerisation_h'] = pd.to_numeric(df_total['duree_numerisation_h'], errors='coerce')

    # v3.1 DEBUG
    n_duree = df_total['duree_numerisation_h'].notna().sum()
    print(f"🔎 DEBUG duree_numerisation_h non-null au final : {n_duree}")
